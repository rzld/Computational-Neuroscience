...
b_strength = numpy.zeros(len(input_b))
...
for a in range(len(input_b)):
    ...
    while ts[-1] < t:
        ie = 0
        rt = r_init + b * math.sin(2 * math.pi * f * ts[-1])
        ...
        for i in range(N):
            ...
            # poisson process
            rand_num = random.uniform(0, rt*2)
...
plt.plot(input_b, b_strength)
plt.xlabel("Input B")
plt.ylabel("Synaptic strength")
plt.title("Synaptic Strength as Function of B")
plt.show()